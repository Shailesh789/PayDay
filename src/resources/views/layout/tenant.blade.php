...
@section('master')
    ...
    @section('top-bar')
        <app-top-navigation-bar logo-icon-src="{{ $logo_icon }}" :profile-data="{{ json_encode($top_bar_menu) }}"
            :has-work-shift="{{ $hasDefaultWorkShift ? 'true' : 'false' }}">
        </app-top-navigation-bar>
    @show

    @section('side-bar')
        <sidebar :data="{{ json_encode($permissions) }}" logo-src="{{ $logo }}" logo-icon-src="{{ $logo_icon }}"
    ...
    @if (env('IS_DEMO', false))
        <div class="" style="z-index:201; bottom:0; right:0; position:fixed; margin:50px;">
            <a href="https://codecanyon.net/item/payday-hrm-solutions/33681719"
