...
export const FormMixin = {
  ...
  methods: {
     ...
     */
    getEditData(url) {
      if (this.isFunction(this.beforeGetEditData)) this.beforeGetEditData();
    ...
    submitFromFixin(reqType, url, submitData) {
      ...
      if (this.isValidForm()) {
        ...
        if (this.isFunction(this.beforeSubmit)) this.beforeSubmit();
        ...
        axioscall
          .then((response) => {
            ...
            if (this.isFunction(this.afterSuccess)) this.afterSuccess(response);
          })
          .catch(({ response }) => {
            //trigger after error
            if (this.isFunction(this.afterError)) this.afterError(response);
          })
          .finally(() => {
        ...
        if (this.isFunction(this.afterSubmit)) this.afterSubmit();
      } else {
          ...
          comp.$children.filter((smComp) => {
            if (smComp.$options.name === "AppInput") childs.push(smComp);
              ...
              smComp.$children.filter((miniComp) => {
                if (miniComp.$options.name === "AppInput")
                  childs.push(miniComp);
              });
            }
          });
        }
    ...
    isValidForm() {
      ...
      childComponents.filter((item) => {
        ...
        if (item.required && (!item.value || item.value.length === 0)) {
          ...
          if (item.specialValidation && this.isValidPassword(item.value)) {
            if (!_.isEmpty(item.sameAs)) {
              ...
              if (instance.fields[item.sameAs] !== item.value) {
                invalidFields.push(item);
                instance.makeFieldStatusObject(
                  item,
                  message,
                  "sameAs",
                  "sameAs"
          ...
          message = `${item.name} ${this.$t("can_not_before")} ${
            item.notBefore
          }`;
          if (
            !instance.isSecondDateAfterFirstDate(
              instance.fields[item.notBefore],
              item.value
            )
          ) {
            invalidFields.push(item);
          ...
          if (itemLength && item.minLength && itemLength < item.minLength) {
            ...
            item.alphanumeric &&
            !instance.isAlphanumeric(item.value)
          ) {
